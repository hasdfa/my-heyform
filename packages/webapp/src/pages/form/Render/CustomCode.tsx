...
const MicrosoftClarity: FC<{ trackCode: string }> = ({ trackCode }) => {
  return (
    <ScriptElement
      html={`
        <script type="text/javascript">
            ...
            })(window, document, "clarity", "script", "${trackCode}");
        </script>
      `}
    />
  )
...
export const CustomCode = ({ form, query }: { form: FormModel; query: Record<string, any> }) => {
  ...
  return (
    <>
      ...
      {helper.isValid(integrations.microsoftclarity) && (
        <MicrosoftClarity trackCode={integrations.microsoftclarity} />
      )}
    </>
  )
}
